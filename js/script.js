...
// Function to get the forecast by city ID
...
// Function to update the current weather on the UI
...
let dayOfWeek = (dt = new Date().getTime()) => {
  return new Date(dt).toLocaleDateString("ru-RU", { weekday: "long" });
};
...
// Initialize the default city weather (Dubai)
let init = () => {
  weatherForCity("Дубай").then(() => (document.body.style.filter = "blur(0)"));
};
init();

// Search event listener for "Enter" key
...
// Search input event listener for city suggestions
